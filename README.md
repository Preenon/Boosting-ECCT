...
## Contents
    * boosted_ECCT_training_notebook.ipynb: Trains baseline ECCT models using the official ECCT implementation.
    * boosted_ECCT_boosting_notebook.ipynb: Loads trained models, applies selected boosting methodologies, and performs BER/FER evaluations.
    * results/: Contains sample boosted models and BER/FER plots from the paper.
...
This notebook will:
    ...
    1. clone the ECCT repository
    2. train a 6-layer, 32-dimensional ECCT model by default
    3. save the model to your mounted Google Drive
...
This notebook will:
    ...
    1. load the baseline model(s) from Drive (it will automatically select the most recent directory
    2. perform UC-based boosting, block-wise scheduling, dynamic weight sharing, and data augmentation
    3. evaluate BER across a configurable Eb/N0 range
    4. save boosted models and BER plots to Drive

To sweep hyperparameters (epochs_post, target_uc), edit the designated lists in the CONFIG cell.

## Configuration

Key experiment settings (modify as desired):
...
Error-Correcting Code Transformer (ECCT) by Y. Choukroun and L. Wolf (2022)  
Paper: https://arxiv.org/abs/2203.14966  
Code: https://github.com/yoniLc/ECCT

Boosting Neural Min-Sum Decoders by H.-Y. Kwak et al. (2025)  
